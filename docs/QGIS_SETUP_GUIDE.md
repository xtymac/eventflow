...
> 详细技术文档请参阅 [REALTIME_MAP_SYNC.md](./REALTIME_MAP_SYNC.md)

### 功能概述

当您在 QGIS 中编辑道路数据并保存后，EventFlow 网站会**实时更新**，无需任何额外操作。

### 使用流程

```
1. 在 QGIS 中编辑道路 → 2. 保存 (Ctrl+S) → 3. 刷新浏览器 (F5) → 4. 看到最新数据
```
...
3. **查看更新**
   - 打开 https://eventflow.uixai.org/
   - 按 **F5** 刷新页面（或 Ctrl+Shift+R 强制刷新）
   - 地图立即显示最新的道路位置
...
| 注意点 | 说明 |
|--------|------|
| 必须刷新页面 | 仅平移/缩放不会加载新数据 |
| 建议强制刷新 | Ctrl+Shift+R (Windows) / Cmd+Shift+R (Mac) |
| 多用户编辑 | 以最后保存者为准，刷新可见他人编辑 |
