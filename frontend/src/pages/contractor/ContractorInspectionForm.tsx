...
import { useCreateInspection, useUpdateInspection } from '../../hooks/useApi';
...
interface ContractorInspectionFormProps {
  facilityId: string;
  open: boolean;
  onClose: () => void;
  onSubmitted: () => void;
...
/* ── Rank options with colored dots ── */
const RANK_OPTIONS = [
  { value: 'A', label: 'A', description: '健全', dotColor: '#22C55E' },
  { value: 'B', label: 'B', description: '要観察', dotColor: '#FACC15' },
  { value: 'C', label: 'C', description: '要注意', dotColor: '#F87171' },
  { value: 'D', label: 'D', description: '危険', dotColor: '#6B7280' },
];

/* ── Rank select with colored dot ── */
function RankSelectItem({ option }: { option: typeof RANK_OPTIONS[number] }) {
  return (
...
export function ContractorInspectionForm({
  facilityId,
  open,
  onClose,
  onSubmitted,
  ...
  const { control, handleSubmit, reset } = useForm<InspectionFormData>({
    defaultValues: {
      inspectionDate: initialData?.inspectionDate || today,
      inspector: initialData?.inspector || inspectorName,
      structureRank: initialData?.structureRank || '',
      wearRank: initialData?.wearRank || '',
      structureMaterialNotes: initialData?.structureMaterialNotes || '',
      wearMaterialNotes: initialData?.wearMaterialNotes || '',
    },
  });

  const handleSave = async (data: InspectionFormData, status: 'draft' | 'submitted') => {
    if (!isAuthenticated) {
      showNotification({
        title: 'ログインが必要です',
      ...
      if (isEditMode) {
        await updateInspection.mutateAsync({
          ...
          data: {
            ...
            result: status === 'draft' ? '' : '定期点検',
            conditionGrade: (data.structureRank || undefined) as 'A' | 'B' | 'C' | 'D' | 'S' | undefined,
            inspector: data.inspector,
            measurements: {
              wearRank: data.wearRank || undefined,
              wearMaterialNotes: data.wearMaterialNotes || undefined,
              structureRank: data.structureRank || undefined,
              structureMaterialNotes: data.structureMaterialNotes || undefined,
            },
            ...(mediaUrls ? { mediaUrls } : {}),
            status,
          },
    ...
    const msg = status === 'draft'
      ? { title: '下書きを保存しました', message: `定期点検 — ${data.inspectionDate}`, color: 'blue' as const }
      : { title: isEditMode ? '点検記録を更新しました' : '点検記録を保存しました', message: `定期点検 — ${data.inspectionDate}`, color: 'green' as const };
    showNotification(msg);
    onSubmitted();
  };

  const onSubmit = (data: InspectionFormData) => handleSave(data, 'submitted');
  ...
  return (
    <>
      ...
      <div
        ...
        onClick={onClose}
        data-testid="contractor-inspection-backdrop"
      />
      ...
      <div
        ...
        data-testid="contractor-inspection-form"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-4">
          <h2 className="text-xl font-medium text-[#0a0a0a]">{isEditMode ? '点検を編集' : '点検'}</h2>
          <button
            type="button"
            onClick={onClose}
            ...
            aria-label="閉じる"
            data-testid="contractor-inspection-form-close"
          >
            <X className="size-4" />
          </button>
        </div>
        ...
        >
                ...
                render={({ field }) => (
                  ...
                  >
                    <SelectTrigger
                      ...
                      data-testid="inspection-inspector-input"
                    >
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
          ...
          {/* Row 2: Structure Rank | Wear Rank */}
          <div className="grid grid-cols-2 gap-7">
            <div className="flex flex-col gap-1">
              <Label className="text-sm font-medium text-[#0a0a0a]">
                構造ランク
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    <SelectTrigger
                      className="w-full"
                      data-testid="inspection-structure-rank-select"
                    >
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    <SelectTrigger
                      className="w-full"
                      data-testid="inspection-wear-rank-select"
                    >
                      <SelectValue placeholder=" " />
                    </SelectTrigger>
                    <SelectContent>
          ...
          {/* Row 4: Structure Notes | Wear Notes */}
          <div className="grid grid-cols-2 gap-7">
            <div className="flex flex-col gap-1">
              <Label className="text-sm font-medium text-[#0a0a0a]">
                構造部材備考
              ...
              <Controller
                ...
                render={({ field }) => (
                  <textarea
                    ...
                    data-testid="inspection-structure-notes"
                  />
                )}
              />
            </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <textarea
                    ...
                    name={field.name}
                    className="border-input placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 flex min-h-[100px] w-full rounded-md border bg-transparent px-3 py-2 text-base shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 md:text-sm resize-none"
                    data-testid="inspection-wear-notes"
                  />
                )}
              />
            </div>
          ...
          {/* Row 5: Photo upload with thumbnails */}
          ...
          {/* Photo thumbnails */}
          {photos.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-1" data-testid="photo-thumbnails">
              {photos.map((file, i) => (
                <PhotoThumbnail
                  key={`${file.name}-${i}`}
                  file={file}
        ...
        <div className="flex items-center justify-between border-t border-[#e5e5e5] px-4 py-3">
          <Button
            ...
            data-testid="contractor-inspection-clear"
          >
            すべてクリア
          </Button>
          <div className="flex gap-2">
            <Button
              ...
              variant="outline"
              onClick={onDraft}
              data-testid="contractor-inspection-draft"
            >
              下書き保存
            </Button>
            <Button
              type="submit"
              form="contractor-inspection-form"
              data-testid="contractor-inspection-submit"
            >
              提出
            </Button>
          </div>
