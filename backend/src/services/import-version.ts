 ...
 * Features without an ID get an auto-generated one (prefix: "auto_").
 * Returns the number of IDs that were auto-generated.
 */
function ensureFeatureIds(features: Feature[]): number {
  let autoGenCount = 0;
  for (const feature of features) {
    if (!feature.properties) feature.properties = {};
    if (!feature.properties.id) {
      feature.properties.id = `auto_${nanoid()}`;
      feature.properties._autoGeneratedId = true;
      autoGenCount++;
    }
  }
  return autoGenCount;
...
export class ImportVersionService {
  ...
  async validateGeoJSON(versionId: string): Promise<ValidationResult> {
    ...
    for (let i = 0; i < features.length; i++) {
      ...
      // Validate: Missing ID (WARNING - auto-generated IDs will be used)
      if (!featureId) {
        missingIdCount++;
      } else {
    ...
    // Add summary warning for missing IDs (auto-generated IDs will be used)
    if (missingIdCount > 0) {
      warnings.push({
        featureIndex: -1,
        message: `${missingIdCount} features missing id - auto-generated IDs will be used (treated as new additions)`,
      });
    }
   ...
   */
  async getRoadsInScopeWithGeometry(scope: string): Promise<{ id: string; name: string | null; roadType: string | null; ward: string | null; lanes: number | null; direction: string | null; status: string | null; dataSource: string | null; geometryJson: Geometry | null }[]> {
    const baseSelect = {
      id: roadAssets.id,
   ...
   */
  async getRoadsByIds(roadIds: string[]): Promise<{ id: string; name: string | null; roadType: string | null; ward: string | null; lanes: number | null; direction: string | null; status: string | null; dataSource: string | null; geometryJson: Geometry | null }[]> {
  ...
  async generateDiff(versionId: string): Promise<DiffResult> {
    ...
    // Auto-generate IDs for features that don't have one
    const autoGenCount = ensureFeatureIds(importFeatures);
    if (autoGenCount > 0) {
      console.log(`[Import] Auto-generated IDs for ${autoGenCount} features without id property`);
    }
    ...
    let currentRoads: { id: string; name: string | null; roadType: string | null; ward: string | null; lanes: number | null; direction: string | null; status: string | null; dataSource: string | null; geometryJson: Geometry | null }[];
    ...
    if (added.length > 0) {
      ...
      // Skip geometry matching for auto-generated ID features (they are explicitly new)
      const autoGenFeatures = added.filter(f => f.properties?._autoGeneratedId === true);
      const matchableAdded = added.filter(f => f.properties?._autoGeneratedId !== true);

      if (matchableAdded.length > 0 && currentRoads.length > 0) {
        // ============================================================
        ...
        // ============================================================
        if (baselineResult) {
          console.log('[Import] Self-comparison left', matchableAdded.length, 'unmatched features - running geometry matching against', currentRoads.length, 'DB roads');
        } else {
          console.log('[Import] No baseline found. Running geometry matching for', matchableAdded.length, 'unmatched features against', currentRoads.length, 'DB roads');
        }
        if (autoGenFeatures.length > 0) {
          console.log('[Import] Skipping geometry matching for', autoGenFeatures.length, 'auto-generated ID features (treated as new)');
        }
        ...
        let exactCount = 0;
        for (const feature of matchableAdded) {
        ...
        added.length = 0;
        added.push(...afterFuzzy, ...autoGenFeatures);
      ...
      } else if (autoGenFeatures.length > 0) {
        // No matchable features but we have auto-generated ones - keep them as added
        added.length = 0;
        added.push(...autoGenFeatures);
      }
    }
  ...
  private async findBaselineVersion(
    ...
    for (const candidate of candidates) {
      ...
      // Also check reverse: baseline shouldn't be vastly larger than import
      // If baseline has 10x more features, it's a different scope (e.g., full export vs ward subset)
      const reverseRate = overlap / baselineIds.size;
      const sizeRatio = baselineIds.size / importIds.size;

      if (matchRate > 0.5 && (reverseRate > 0.3 || sizeRatio < 2)) {
  ...
  private hasFeatureChanges(
    feature: Feature,
    existing: { name: string | null; roadType: string | null; ward: string | null; lanes: number | null; direction: string | null; status: string | null; dataSource: string | null; geometryJson: Geometry | null }
  ): boolean {
    const props = feature.properties || {};

    // Compare key fields (trim whitespace for robustness against GeoPackage roundtrip issues)
    const trimOrNull = (v: unknown): string | null => {
      if (v === undefined || v === null) return null;
      return String(v).trim() || null;
    };

    if (props.name !== undefined && trimOrNull(props.name) !== trimOrNull(existing.name)) return true;
    if (props.roadType !== undefined && trimOrNull(props.roadType) !== trimOrNull(existing.roadType)) return true;
    if (props.ward !== undefined && trimOrNull(props.ward) !== trimOrNull(existing.ward)) return true;
    if (props.lanes !== undefined && props.lanes !== existing.lanes) return true;
    if (props.direction !== undefined && trimOrNull(props.direction) !== trimOrNull(existing.direction)) return true;
    if (props.status !== undefined && trimOrNull(props.status) !== trimOrNull(existing.status)) return true;
    if (props.dataSource !== undefined && trimOrNull(props.dataSource) !== trimOrNull(existing.dataSource)) return true;
  ...
  async publishVersion(
    ...
    const geojsonPath = version.filePath.endsWith('.geojson')
      ...
      : join(dirname(version.filePath), 'converted.geojson');
    const geojson = JSON.parse(readFileSync(geojsonPath, 'utf-8')) as FeatureCollection;
    const publishFeatures = geojson.features || [];

    // Auto-generate IDs for features that don't have one (same as in generateDiff)
    ensureFeatureIds(publishFeatures);
